...
namespace Barrage
{
    class ReadString
    {
        ...
        static readonly Dictionary<string, double> opPrecedence = new Dictionary<string, double>() {
        ...
        private static double Solve(string inp)
        {
            ...
            for (int i = 0; i >= 0 && i < input.Count && !MainWindow.stopGameRequested; i++)
            {
                switch (input[i])
                {
                    //1 value operators
                    case "SQRT":
                        double[] nums = GetVals(ref input, inp, i - 1, 1);
                        input[i] = Math.Sqrt(nums[0]);
                        input.RemoveAt(Math.Max(i - 1, 0)); i--;
                        break;
                    case "SIGN":
                        nums = GetVals(ref input, inp, i - 1, 1);
                    ...
                    //2 value operators
                    case "==":
                        nums = GetVals(ref input, inp, i - 2, 2);
